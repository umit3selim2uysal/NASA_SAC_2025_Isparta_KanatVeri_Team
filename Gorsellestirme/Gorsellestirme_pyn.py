...
class MultiChannelDataVisualization(QWidget):
    ...
    def __init__(self):
        ...
        self.data_buffers = {
            ...
            'metan': {'data': [], 'time': [], 'color': 'r'},
            'dogalgaz': {'data': [], 'time': [], 'color': 'g'},
            'lpg': {'data': [], 'time': [], 'color': 'b'},
            'alkol': {'data': [], 'time': [], 'color': 'm'},
            'hidrojen': {'data': [], 'time': [], 'color': 'c'},
            'smoke': {'data': [], 'time': [], 'color': 'orange'}
        }
        self.max_points = 500
        self.start_time = time.time()
        self.last_sensor_data = {}  # Son sensor verilerini sakla
    ...
    def init_ui(self):
        ...
        # Minimum Y aralığı belirle (küçük değişiklikleri görünür yapmak için)
        self.gas_plot.setYRange(-0.5, 10, padding=0.1)
        ...
        # Tüm gaz tiplerini ekle
        self.curves['metan'] = self.gas_plot.plot(pen=pg.mkPen(color='r', width=2), name='Metan')
        self.curves['dogalgaz'] = self.gas_plot.plot(pen=pg.mkPen(color='g', width=2), name='Doğalgaz')
        self.curves['lpg'] = self.gas_plot.plot(pen=pg.mkPen(color='b', width=2), name='LPG')
        self.curves['alkol'] = self.gas_plot.plot(pen=pg.mkPen(color='m', width=2), name='Alkol')
        self.curves['hidrojen'] = self.gas_plot.plot(pen=pg.mkPen(color='c', width=2), name='Hidrojen')
    ...
    def add_sensor_data(self, sensor_data):
        ...
        # Son sensor verilerini kaydet
        self.last_sensor_data.update(sensor_data)
        
        # Debug: Gaz verilerini kontrol et
        gas_values = {}
        for gas_type in ['metan', 'dogalgaz', 'lpg', 'alkol', 'hidrojen', 'smoke']:
            value = sensor_data.get(gas_type, 0)
            if value != 0:  # Sadece sıfır olmayan değerleri logla
                gas_values[gas_type] = value
        if gas_values:
            print(f"DEBUG: Gaz verileri alındı: {gas_values}")
        else:
            print(f"DEBUG: Tüm gaz değerleri sıfır - bu normal, grafikte görünür olmalı")
        ...
        # Gaz konsantrasyonları - sıfır değerleri de dahil et
        for gas_type in ['metan', 'dogalgaz', 'lpg', 'alkol', 'hidrojen', 'smoke']:
            gas_value = sensor_data.get(gas_type, 0)
            # Tüm değerleri ekle (sıfır dahil) - bu normal gaz seviyelerini gösterir
            self.add_data_point(gas_type, gas_value, current_time)
            if gas_value > 0:
                print(f"DEBUG: {gas_type} = {gas_value} ppm eklendi")
    ...
    def add_periodic_update(self):
        """Periyodik güncelleme - son değerleri tekrar ekleyerek grafiği canlı tutar"""
        if self.last_sensor_data:
            current_time = time.time() - self.start_time
            # Sadece gaz verilerini periyodik olarak ekle
            for gas_type in ['metan', 'dogalgaz', 'lpg', 'alkol', 'hidrojen', 'smoke']:
                if gas_type in self.last_sensor_data:
                    value = self.last_sensor_data[gas_type]
                    self.add_data_point(gas_type, value, current_time)
...
class SerialMonitorApp(QMainWindow):
    ...
    def __init__(self):
        ...
        # Grafik güncelleme timer'ı - gaz grafiğini canlı tutmak için
        self.graph_update_timer = QTimer()
        self.graph_update_timer.timeout.connect(self.update_graphs)
        self.graph_update_timer.start(500)  # Her 500ms'de güncelle
    ...
    def update_graphs(self):
        """Grafikleri periyodik olarak güncelle"""
        if hasattr(self, 'visualization_widget'):
            self.visualization_widget.add_periodic_update()
